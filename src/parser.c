...
char* print_hash_literal(void* hash_casted_to_void)
{
    ...
    for (int i = 0; i < 5000; ++i) {
        ...
        while (runner != NULL) {
            ...
            char* value_string = value_expression->node.string((void*)value_expression);
...
{
    hash_literal_t* hash_literal = (hash_literal_t*)malloc(sizeof(hash_literal_t));
    generic_hash_table_t* hash_table = create_hash_table(HASH_KEY_POINTER_TYPE, HASH_VALUE_TYPE_EXPRESSION);
...
{
    index_expression_t* index_expr = (index_expression_t*)index_expr_cast_to_void;
    char* buffer = (char*)malloc(STRING_MAX_SIZE);
    buffer[0] = '\0';
    strcat(buffer, "(");
...
{
    index_expression_t* expr = (index_expression_t*)malloc(sizeof(index_expression_t));
...
expression_t* parse_array_literal(parser_t* parser)
{
    ...
    expression_t** elements = calloc(DEFAULT_DYNAMIC_ARR_SIZE, sizeof(expression_t));
    ...
    expression_t* expression)
{
    if (function_expression->arguments_length + 1 > function_expression->arguments_capacity) {
        function_expression->arguments = realloc(function_expression->arguments,
