...
static integer_obj_t *
new_integer_obj (long long value)
{
...
object_t *
eval_if_expression (if_expression_t *if_expr, environment_t *env)
{
    ...
    if (is_condition_true)
        {
            object_t *eval_consequence
                = eval_statement (if_expr->consequence, env);
    ...
    else
        {
            if (if_expr->alternative == NULL)
                {
                    null_obj_t *null_obj
                        = (null_obj_t *)malloc (sizeof (null_obj_t));
                    null_obj->object.type = type_null;
                    null_obj->object.inspect = inspect_null;
                    return (object_t *)null_obj;
                }
            object_t *eval_alternative
                = eval_statement (if_expr->alternative, env);
...
object_t *
unwrap_return_value (object_t *obj)
{
...
environment_t *
extend_function_env (environment_t *env, object_t **evaluated_expressions,
                     ...
                     function_obj_t *function_obj)
{
    ...
    for (int i = 0; i < evaluated_expressions_length; ++i)
        {
            identifier_t *evaluated_expr
                = (identifier_t *)function_obj->parameters[i];
            set_environment (new_env, evaluated_expr->value,
...
integer_obj_t *
get_monkey_string_length (string_obj_t *argument)
{
    integer_obj_t *ret_integer
        = (integer_obj_t *)malloc (sizeof (integer_obj_t));
...
integer_obj_t *
get_monkey_array_length (array_obj_t *array)
{
    integer_obj_t *ret_integer
        = (integer_obj_t *)malloc (sizeof (integer_obj_t));
    ret_integer->object.type = type_int;
...
object_t *
copy_monkey_object (object_t *original)
{
    switch (original->type ())
        ...
        case ARRAY_OBJ:
            {
                array_obj_t *array_obj
                    = (array_obj_t *)malloc (sizeof (array_obj_t));
                array_obj_t *original_array = (array_obj_t *)original;
                array_obj->elements_length = original_array->elements_length;
                array_obj->elements_capacity
                    = original_array->elements_capacity;
                array_obj->elements = calloc (
                    original_array->elements_capacity, sizeof (object_t));
                for (int i = 0; i < original_array->elements_length; ++i)
                    {
                        array_obj->elements[i]
                            = copy_monkey_object (original_array->elements[i]);
                    }
...
object_t *
apply_function (expression_t *function, object_t **evaluated_expressions,
                size_t evaluated_expressions_length, environment_t *env)
{
    ...
    if (function_literal->type ((void *)function_literal) == BUILTIN_OBJ)
        {
            ...
            if (strcmp (builtin->name, "puts") == 0)
                {
                    string_obj_t *str_obj
                        = (string_obj_t *)malloc (sizeof (string_obj_t));
                    ...
                    for (int i = 0; i < evaluated_expressions_length; ++i)
                        {
                            object_t *evaled_expression
                                = evaluated_expressions[i];
                            strcat (str_obj->value,
                                    evaled_expression->inspect (
                                        (void *)evaled_expression));
            ...
            if (strcmp (builtin->name, "len") == 0)
                {
                    if (evaluated_expressions_length != 1)
                        {
                            ...
                            snprintf (err_string, STRING_MAX_SIZE,
                                      "string/array length function takes in "
                                      "only 1 argument, got %zu\n",
                    ...
                    snprintf (err_string, STRING_MAX_SIZE,
                              ...
                              object_type_strings[evaluated_expressions[0]
                                                      ->type ()]);
                    err->message = err_string;
                    return (object_t *)err;
                }
                    ...
                    if (evaluated_expressions_length != 1)
                        {
                            ...
                            char *err_string
                                = (char *)malloc (STRING_MAX_SIZE);
                            snprintf (err_string, STRING_MAX_SIZE,
                                      "array first function takes in only 1 "
                                      "argument, got %zu\n",
                                      evaluated_expressions_length);
                            err->message = err_string;
                            return (object_t *)err;
                    ...
                    if (evaluated_expressions_length != 1)
                        {
                            ...
                            char *err_string
                                = (char *)malloc (STRING_MAX_SIZE);
                            snprintf (err_string, STRING_MAX_SIZE,
                                      "array last function takes in only 1 "
                                      "argument, got %zu\n",
                                      evaluated_expressions_length);
                            err->message = err_string;
                            return (object_t *)err;
                    ...
                    if (evaluated_expressions[0]->type () == ARRAY_OBJ)
                        {
                            ...
                            return copy_monkey_object (
                                array->elements[array->elements_length - 1]);
                        }
                }
                    ...
                    if (evaluated_expressions_length != 1)
                        {
                            ...
                            char *err_string
                                = (char *)malloc (STRING_MAX_SIZE);
                            snprintf (err_string, STRING_MAX_SIZE,
                                      "array tail function takes in only 1 "
                                      "argument, got %zu\n",
                                      evaluated_expressions_length);
                            err->message = err_string;
                            return (object_t *)err;
            ...
            if (strcmp (builtin->name, "map") == 0)
                {
                    if (evaluated_expressions_length != 2)
                        {
                            ...
                            char *err_string
                                = (char *)malloc (STRING_MAX_SIZE);
                            snprintf (err_string, STRING_MAX_SIZE,
                                      "array map function takes in only 2 "
                                      "arguments, got %zu\n",
                                      evaluated_expressions_length);
                            err->message = err_string;
                            return (object_t *)err;
                    ...
                    if (evaluated_expressions[0]->type () == ARRAY_OBJ
                        && evaluated_expressions[1]->type () == FUNCTION_OBJ)
                        {
                            ...
                            function_obj_t *func
                                = (function_obj_t *)evaluated_expressions[1];
                            array_obj_t *copied_array
                                = (array_obj_t *)copy_monkey_object (
                                    (object_t *)array);
                            copied_array->object.type = type_array;
                            ...
                            if (func->parameters_length == 1)
                                {
                                    for (int i = 0;
                                         i < copied_array->elements_length;
                                         ++i)
                                        {
                                            environment_t *new_env
                                                = extend_function_env (
                                                    env, &array->elements[i],
                                                    1, func);
                                            copied_array->elements[i]
                                                = unwrap_return_value (
                                                    eval_statements (
                                         ...
                                         ++i)
                                        {
                                            object_t **parameters
                                                = (object_t **)calloc (
                                                    2, sizeof (object_t));
                                            integer_obj_t *integer_obj
                                                = (integer_obj_t *)malloc (
                                                    sizeof (integer_obj_t));
                                            integer_obj->object.type
                                                = type_int;
                                            integer_obj->object.inspect
                                                = inspect_int;
                                            integer_obj->value = i;
                                            parameters[0] = array->elements[i];
                                            parameters[1]
                                                = (object_t *)integer_obj;
                                            environment_t *new_env
                                                = extend_function_env (
                                                    env, parameters, 2, func);
                                            copied_array->elements[i]
                                                = unwrap_return_value (
                                                    eval_statements (
                                                        func->body->statements,
                            ...
                            if (func->parameters_length < 1
                                || func->parameters_length > 2)
                                {
                                    error_obj_t *err = create_error_object ();
                                    char *err_string
                                        = (char *)malloc (STRING_MAX_SIZE);
                                    snprintf (err_string, STRING_MAX_SIZE,
                                              "function given as map argument "
                                              "should have only 1 or 2 "
                                              "arguments");
                                    err->message = err_string;
                                    return (object_t *)err;
                                }
                    ...
                    if (evaluated_expressions_length != 2)
                        {
                            ...
                            char *err_string
                                = (char *)malloc (STRING_MAX_SIZE);
                            snprintf (err_string, STRING_MAX_SIZE,
                                      "array push function takes in only 2 "
                                      "arguments, got %zu\n",
                                      evaluated_expressions_length);
                            err->message = err_string;
                            return (object_t *)err;
                    ...
                    if (evaluated_expressions[0]->type () == ARRAY_OBJ)
                        {
                            array_obj_t *array
                                = (array_obj_t *)evaluated_expressions[0];
                            if (array->elements_length + 1
                                > array->elements_capacity)
                                {
                            ...
                            array->elements[array->elements_length++]
                                = evaluated_expressions[1];
                            return (object_t *)create_null_obj ();
                        }
                }
        }
    ...
    if (function_typecasted->parameters_length != evaluated_expressions_length)
        {
            ...
            char *err_string = (char *)malloc (STRING_MAX_SIZE);
            snprintf (err_string, STRING_MAX_SIZE,
                      "Function expected %zu arguments got %zu",
                      function_typecasted->parameters_length,
                      evaluated_expressions_length);
            err->message = err_string;
            return (object_t *)err;
        }
    environment_t *extended_environment = extend_function_env (
        function_typecasted->env, evaluated_expressions,
...
object_t *
eval_call_expression (call_expression_t *call_expression, environment_t *env)
{
...
object_t *
eval_string_expression (string_literal_t *string_literal, environment_t *env)
{
...
function_obj_t *
eval_function_expression (function_literal_t *function_literal,
                          environment_t *env)
{
    function_obj_t *func_obj
        = (function_obj_t *)malloc (sizeof (function_obj_t));
...
object_t *
eval_infix_expression (infix_expression_t *infix_expr, environment_t *env)
{
    if (strcmp (infix_expr->op, "+") == 0)
        {
            ...
            if (left_obj->type () != INTEGER || right_obj->type () != INTEGER)
                {
                    if (left_obj->type () == STRING_OBJ
                        && right_obj->type () == STRING_OBJ)
                        {
                            string_obj_t *left_string
                                = (string_obj_t *)left_obj;
                            string_obj_t *right_string
                                = (string_obj_t *)right_obj;
                            string_obj_t *return_obj = (string_obj_t *)malloc (
                                sizeof (string_obj_t));
                            char *added_string
                                = (char *)malloc (2 * STRING_MAX_SIZE);
                            added_string[0] = '\0';
                            return_obj->value
                                = strcat (added_string, left_string->value);
                            return_obj->value
                                = strcat (added_string, right_string->value);
    ...
    else if (strcmp (infix_expr->op, "==") == 0)
        {
            object_t *left_obj = eval_expression (infix_expr->left, env);
            object_t *right_obj = eval_expression (infix_expr->right, env);
            bool are_both_ints = left_obj->type () == INTEGER
                                 && right_obj->type () == INTEGER;
            bool are_both_bools = right_obj->type () == BOOLEAN
                                  && right_obj->type () == BOOLEAN;
            ...
            boolean_obj_t *bool_obj
                = (boolean_obj_t *)malloc (sizeof (boolean_obj_t));
            if (are_both_ints)
                {
    ...
    else if (strcmp (infix_expr->op, "<=") == 0)
        {
            object_t *left_obj = eval_expression (infix_expr->left, env);
            object_t *right_obj = eval_expression (infix_expr->right, env);
            boolean_obj_t *bool_obj = malloc (sizeof (boolean_obj_t));
            bool are_both_ints = left_obj->type () == INTEGER
                                 && right_obj->type () == INTEGER;
            if (!are_both_ints)
                {
                    ...
                    char *error_string = (char *)malloc (STRING_MAX_SIZE);
                    snprintf (error_string, STRING_MAX_SIZE,
                              "Cannot compare non integers %d", __LINE__);
                    error_obj->message = error_string;
                    return (object_t *)error_obj;
                }
            bool_obj->value = (((integer_obj_t *)left_obj)->value
...
object_t *
eval_prefix_expression (prefix_expression_t *prefix_expr, environment_t *env)
{
    object_t *right = eval_expression (prefix_expr->right, env);
    if (strcmp (prefix_expr->op, "!") == 0)
        {
            // TODO: refactor: make into seperate functions
            boolean_obj_t *boolean_obj
                = (boolean_obj_t *)malloc (sizeof (boolean_obj_t));
            switch (right->type ())
                {
                case BOOLEAN:
                    {
                        boolean_obj_t *boolean_obj_right
                            = (boolean_obj_t *)right;
                        boolean_obj->value
                            = boolean_obj_right->value == true ? false : true;
                        break;
                    }
                case NULL_OBJ:
                    {
                ...
                case INTEGER:
                    {
                        ...
                        boolean_obj->value
                            = integer_obj->value == 0 ? true : false;
                        break;
                    }
                default:
                    {
...
array_obj_t *
eval_array_literal (array_literal_t *arr_literal, environment_t *env)
{
    ...
    array_obj->elements = (object_t **)calloc (arr_literal->elements_capacity,
                                               sizeof (object_t));
    for (int i = 0; i < arr_literal->elements_length; i++)
        {
            array_obj->elements[i]
                = eval_expression (arr_literal->elements[i], env);
        }
    return array_obj;
}

object_t *
eval_index_expr (index_expression_t *index_expr, environment_t *env)
{
...
hash_obj_t *
eval_hash_literal (hash_literal_t *hash_literal, environment_t *env)
{
    ...
    hash_obj->pairs = create_hash_table (HASH_KEY_HASH_KEY_TYPE,
    ...
    for (int i = 0; i < 5000; ++i)
        {
            ...
            while (runner != NULL)
                {
                    generic_key_value_t *key_val
                        = (generic_key_value_t *)runner->data;
                    object_t *evaled_key
                        = eval_expression ((expression_t *)key_val->key, env);
                    object_t *evaled_value = eval_expression (
                        (expression_t *)key_val->value, env);
                    hash_key_t *hash_key = hash_object (evaled_key);
                    hash_pair_t *hash_pair
                        = (hash_pair_t *)malloc (sizeof (hash_pair_t));
...
object_t *
eval_expression (expression_t *expression, environment_t *env)
{
    switch (expression->type)
                ...
                if (val == NULL)
                    {
                        if (strcmp (identifier->value, "len") == 0
                            ...
                            {
                                builtin_obj_t *builtin
                                    = (builtin_obj_t *)malloc (
                                        sizeof (builtin_obj_t));
                        ...
                        snprintf (err_obj->message, STRING_MAX_SIZE,
        ...
        case FUNCTION_EXPRESSION:
            {
                function_literal_t *func_expression
                    = (function_literal_t *)expression;
                return (object_t *)eval_function_expression (func_expression,
                                                             env);
            }
        ...
        case STRING_LITERAL:
            {
                ...
                return (object_t *)eval_string_expression (string_literal,
                                                           env);
            }
        case ARRAY_LITERAL:
            {
...
object_t *
eval_block_statement (block_statement_t *block_statement, environment_t *env)
{
    object_t *result;
...
object_t *
eval_statements (statement_t **statements, size_t statements_length,
                 environment_t *env)
{
    object_t *result;
        ...
        case LET_STATEMENT:
            {
                ...
                return (object_t *)
                    create_null_obj (); // FIXME: hack to avoid early returns
                                        // during function calls
            }
        case RETURN_STATEMENT:
            {
                ret_statement_t *ret_stmt = (ret_statement_t *)statement;
                return_obj_t *ret_obj
                    = (return_obj_t *)malloc (sizeof (return_obj_t));
...
void
free_program_node (program_t *program_node)
{
...
object_t *
eval (node_t *node, environment_t *env)
{
