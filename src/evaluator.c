...
{
    function_obj_t* function_obj = (function_obj_t*)malloc(sizeof(function_obj_t));
    return function_obj;
}
...
object_t* copy_monkey_object(object_t* original)
{
    ...
    case ARRAY_OBJ: {
        ...
        array_obj->elements = calloc(original_array->elements_capacity, sizeof(object_t));
    ...
    environment_t* env)
{
    ...
    if (function_literal->type((void*)function_literal) == BUILTIN_OBJ) {
        ...
        if (strcmp(builtin->name, "tail") == 0) {
            ...
            if (evaluated_expressions[0]->type() == ARRAY_OBJ) {
                ...
                array_obj_t* copied_array = (array_obj_t*)copy_monkey_object((object_t*)array);
        ...
        if (strcmp(builtin->name, "map") == 0) {
            ...
            if (evaluated_expressions[0]->type() == ARRAY_OBJ && evaluated_expressions[1]->type() == FUNCTION_OBJ) {
                array_obj_t* array = ((array_obj_t*)evaluated_expressions[0]);
                if (array->elements_length == 0) {
                    return (object_t*)create_null_obj();
                }
                ...
                if (func->parameters_length == 1) {
                    for (int i = 0; i < copied_array->elements_length; ++i) {
                        environment_t* new_env = extend_function_env(env, &array->elements[i], 1, func);
                ...
                if (func->parameters_length == 2) {
                    for (int i = 0; i < copied_array->elements_length; ++i) {
                        ...
                        environment_t* new_env = extend_function_env(env, parameters, 2, func);
                        copied_array->elements[i] = unwrap_return_value(
                            eval_statements(func->body->statements,
                                func->body->statements_length, new_env));
                    }
        ...
        if (strcmp(builtin->name, "push") == 0) {
            ...
            if (evaluated_expressions[0]->type() == ARRAY_OBJ) {
                ...
                if (array->elements_length + 1 > array->elements_capacity) {
                    ...
                    array->elements = realloc(array->elements, array->elements_capacity * sizeof(object_t));
                }
    ...
    environment_t* extended_environment = extend_function_env(function_typecasted->env, evaluated_expressions,
    ...
    environment_t* env)
{
    ...
    for (int i = 0; i < call_expression->arguments_length; ++i) {
        evaluated_expressions[i] = eval_expression(call_expression->arguments[i], env);
    }
    ...
    environment_t* env)
{
    ...
    array_obj->elements = (object_t**)calloc(arr_literal->elements_capacity, sizeof(object_t));
    ...
    environment_t* env)
{
    ...
    hash_obj->pairs = create_hash_table(HASH_KEY_HASH_KEY_TYPE, HASH_VALUE_TYPE_HASH_PAIR);
    ...
    for (int i = 0; i < 5000; ++i) {
        ...
        while (runner != NULL) {
            ...
            object_t* evaled_value = eval_expression((expression_t*)key_val->value, env);
...
object_t* eval_expression(expression_t* expression, environment_t* env)
{
    ...
    case IDENTIFIER: {
        ...
        if (val == NULL) {
            if (strcmp(identifier->value, "len") == 0 || strcmp(identifier->value, "first") == 0 || strcmp(identifier->value, "last") == 0 || strcmp(identifier->value, "tail") == 0 || strcmp(identifier->value, "map") == 0 || strcmp(identifier->value, "push") == 0) {
