...
generic_linked_list_t* create_linked_list(linked_list_type type)
{
    generic_linked_list_t* ll = (generic_linked_list_t*)malloc(sizeof(generic_linked_list_t));
    ll->type = type;
    return ll;
}
...
{
    generic_hash_table_t* hash_table = (generic_hash_table_t*)malloc(sizeof(generic_hash_table_t));
    ...
    for (int i = 0; i < HASH_TABLE_SIZE; ++i) {
        hash_table->items[i] = (generic_linked_list_t*)malloc(sizeof(generic_linked_list_t));
        hash_table->items[i]->type = LL_KEY_VALUE_TYPE;
    }
    return hash_table;
}

uint64_t hash_hash_key_t(hash_key_t* key) { return hash_pointer(key->value); }

void push_linked_list(generic_linked_list_t* ll, void* value)
{

    generic_linked_list_node_t* ll_node = (generic_linked_list_node_t*)malloc(sizeof(generic_linked_list_node_t));
    ...
    void* value)
{
    ...
    generic_key_value_t* key_value = (generic_key_value_t*)malloc(sizeof(generic_key_value_t));
...
void free_ll_node(generic_linked_list_node_t* ll_node)
{
    FREE(ll_node->data);
    FREE(ll_node);
}

void free_ll(generic_linked_list_t* ll)
{
...
void free_hash_table(generic_hash_table_t* hash_table)
{
    for (int i = 0; i < HASH_TABLE_SIZE; ++i) {
...
bool key_equals(hash_key_type type, void* key1, void* key2)
{
    switch (type) {
    case HASH_KEY_POINTER_TYPE: {
...
generic_linked_list_search(generic_linked_list_t* ll, void* key)
{
    ...
    generic_linked_list_node_t* runner = ll->head;
    // TODO: This needs a more black boxish implementation, not doing it for now since the scope of this DS is only for hashmaps
    switch (runner->type) {
    case LL_KEY_VALUE_TYPE: {
        generic_key_value_t* key_value = (generic_key_value_t*)runner->data;
        while (runner != NULL && !key_equals(key_value->key_type, key_value->key, key)) {
            runner = runner->next;
        }
        return runner;
    }
...
void* get_value_hash_table(generic_hash_table_t* hash_table, void* key)
{
    ...
    generic_linked_list_node_t* node = generic_linked_list_search(hash_table->items[hash_key], key);
