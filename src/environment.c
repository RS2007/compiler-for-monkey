...
{
    generic_key_value_t* key_value = (generic_key_value_t*)get_value_hash_table(environment->store, key);
    if (key_value == NULL) {
        return NULL;
    }
...
object_t* set_environment(environment_t* environment, char* key,
    object_t* value)
{
    char* duplicated_key = strdup(key); // Duplication needed since expression gets freed on execution
